//
//  NavigationDefaultSegmentDemoViewController.swift
//  FlowNest
//
...
enum NavigationDefaultSegmentDemo {
    ...
    static func makeViewController() -> FlowNestContainerViewController {
        ...
        viewController.headerView = makeHeaderView(title: "Default Segment", subtitle: "容器内导航栏 + 默认 Segment")
    ...
    private static func makeHeaderView(title: String, subtitle: String) -> UIView {
        ...
        let card = UIView(frame: CGRect(x: 20, y: 152, width: 188, height: 44))
        card.backgroundColor = UIColor(white: 1.0, alpha: 0.12)
        card.layer.cornerRadius = 12
        
        let cardLabel = UILabel(frame: CGRect(x: 14, y: 11, width: 160, height: 22))
        cardLabel.text = "支持点击和左右滑动"
        cardLabel.textColor = .white
        cardLabel.font = UIFont.systemFont(ofSize: 14, weight: .medium)
